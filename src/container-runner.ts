 ...
 * Spawns the agent-runner as a local Node.js process (no Docker).
 * The agent inherits the host environment (AWS credentials, tools, etc).
 */
...
/**
 * Prepare local directories and settings for the agent process.
 * Returns environment variables that tell the agent-runner where everything is.
 */
function prepareAgentEnvironment(
  ...
  // Ensure PATH includes common tool locations that launchd may not have
  ...
  // via project-level config (cwd/.claude/), while HOME stays real for ~/.aws etc.
  const groupClaudeLink = path.join(groupDir, '.claude');
  if (!fs.existsSync(groupClaudeLink)) {
    fs.symlinkSync(groupSessionsDir, groupClaudeLink);
  }
  ...
  if (envConfig.CLAUDE_CODE_USE_BEDROCK === '1') {
    env.CLAUDE_CODE_USE_BEDROCK = '1';
    if (envConfig.AWS_PROFILE) env.AWS_PROFILE = envConfig.AWS_PROFILE;
    if (envConfig.AWS_REGION) env.AWS_REGION = envConfig.AWS_REGION;
  }
...
export async function runContainerAgent(
  ...
  const agentEnv = prepareAgentEnvironment(group, input.isMain);
